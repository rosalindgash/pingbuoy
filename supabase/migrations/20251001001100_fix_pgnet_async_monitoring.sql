-- Fix monitoring function to properly use pg_net extension asynchronously
-- pg_net.http_get returns request_id, actual response is in net.http_request_queue
...
BEGIN
    ...
    LOOP
        BEGIN
            -- Make HTTP request using pg_net - this returns a request_id
            SELECT net.http_get(
                site_record.url,
            ...
            -- Wait for the request to complete (poll the queue)
            wait_count := 0;
            LOOP
            ...
            response_time := EXTRACT(EPOCH FROM (http_response.created - check_time))::INTEGER * 1000;

            -- Determine if site is up based on status code
...
-- Update comments
COMMENT ON FUNCTION real_tiered_uptime_monitoring_with_ssl IS 'Async monitoring using pg_net extension with SSL checks via Edge Function. Runs every 3 minutes via cron. Pro/Founder: 3min intervals, Free: 10min intervals';